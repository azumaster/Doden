(function (window) {
  ...
  * */
  ...
  req.userPermission = (data = {}, callback) => {
    return send({type: 'get', url: '/mock/shop/permission_member.json', data: data, callback: callback});
  };
  // 设置/取消权限
  req.editPermission = (data = {}, callback) => {
    return send({type: 'get', url: '/mock/shop/permission_member_edit.json', data: data, callback: callback});
  };
  window.$.req = req;
})(window);
