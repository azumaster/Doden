...
;(function (global,factory) {
    ...
    H.sheet = (function() {
        ...
        * */
         ...
         * options参数说明:
         * */
        function sendGoodsConsultNotice(options) {
            create({
                title: '咨询',
                content: '<div class="actionsheet_cell read-only">' +
                '<div class="actionsheet_item" style="padding: 10px 80px"><input id="ConsultGoodsNum" type="tel" style="height: 30px;width: 100%;' +
                'border-bottom: 1px solid #bfbfbf;font-size: 14px;text-align: center" placeholder="请输入想购买的数量"/></div>'+
                '<div class="actionsheet_item center">商品：'+options.title+'</div>'+
                '<div class="actionsheet_item center">商家：'+options.seller_name+'('+options.market+')'+'</div>'+
                '<div class="actionsheet_item center" style="color: #888;">咨询后在线下单，售后服务、交易纠纷将由平台解决。</div>'+
                '</div>',
                cancel: '关闭',
                confirm: '拨打',
                confirmCallback: function() {
                    var val = $('#ConsultGoodsNum').val();

                    if(!val) {
                        $('#ConsultGoodsNum')[0].focus();
                        return;
                    }
                    var param = {
                        goods_id: options.id,
                    ...
                    H.we_loading.show();
                    H.server.sendGoodsConsultNotice(param, (res) => {
                        H.we_loading.hide();
                        if(res.code == 0) {
                            window.location.href = 'tel:'+options.tel;
                        }else {
            ...
            var dom = $('#ConsultGoodsNum'),
                val = null;
            dom.bind('keydown', function() {
                val = dom.val();
            });
        ...
        return {
            ...
            addCar: addCar,
            sendGoodsConsultNotice: sendGoodsConsultNotice
        }
    })();
